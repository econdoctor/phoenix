...
exit();}
if($type == '2'){
$sql_del = "DELETE FROM phoenix_permissions_topics WHERE permission_id = '".$perm_id."'";
$res_del = $mysqli -> query($sql_del);}
$sql_type = "UPDATE phoenix_permissions SET permission_type = '1' WHERE permission_id = '".$perm_id."'";
$res_type = $mysqli -> query($sql_type);
if(!array_filter($_POST['permission_paper'])){
echo 'Please select at least one paper.';
exit();}
if(!empty($_POST['permission_paper'])){
foreach($_POST['permission_paper'] as $check){
$sqlx = "INSERT INTO phoenix_permissions_papers (permission_id, paper_id) VALUES ('".$perm_id."', '".$check."')";
