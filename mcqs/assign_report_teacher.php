...
if(empty($assign_id) || empty($member_id)){
...
$sql_check = "SELECT assign_student_end FROM phoenix_assign_users WHERE assign_id = '".$assign_id."' AND student_id = '".$member_id."'";
$res_check = $mysqli -> query($sql_check);
$nr_check = mysqli_num_rows($res_check);
$data_check = mysqli_fetch_assoc($res_check);
$assign_end = $data_check['assign_student_end'];
if($nr_check == 0){
...
if($version == 0){
$version = "/";}
$sql8 = "SELECT answer FROM phoenix_answers WHERE user_id = '".$member_id."' AND question_id = '".$question_id."' AND assign_id = '".$assign_id."' AND answer_type = '3'";
