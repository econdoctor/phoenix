...
$team_shield = $_GET['team_shield'];
if(empty($team_shield) || $team_shield == ''){
header("Location: team_shield.php?assign_id=$assign_id&error=1");
exit();}
$sql_a = "SELECT COUNT(*) FROM phoenix_assign_teams WHERE assign_id = '".$assign_id."' AND team <> '".$team."' AND team_shield = '".$team_shield."'";
$res_a = $mysqli -> query($sql_a);
$data_a = mysqli_fetch_assoc($res_a);
$nr_a = $data_a['COUNT(*)'];
if($nr_a > 0){
header("Location: team_shield.php?assign_id=$assign_id&error=2");
exit();}
$team_shield_clear = $mysqli -> real_escape_string($team_shield);
$sql_up = "UPDATE phoenix_assign_teams SET team_shield = '".$team_shield_clear."' WHERE assign_id = '".$assign_id."' AND team = '".$team."'";
