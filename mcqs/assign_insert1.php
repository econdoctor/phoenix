...
if($ongoing >= 10){
header("Location: assign_new.php?error=7");
exit();}}
...
header("Location: assign_new.php?error=1&s=$s&n=$n&t=$t&ta=$ta&start_d=$start_d&start_t=$start_t&deadline_d=$deadline_d&deadline_t=$deadline_t&start=$start_dtl&deadline=$deadline_dtl");
exit();}
if($t == '4' || $t == "5"){
...
exit();}
if($t == "5"){
header("Location: assign_students.php?assign_id=$assign_id");
exit();}}
if($t != '4' && $t != '5'){
...
header("Location: assign_new.php?error=1&s=$s&n=$n&t=$t&ta=$ta&start_d=$start_d&start_t=$start_t&deadline_d=$deadline_d&deadline_t=$deadline_t");
exit();}
if(!empty($start_date_f) && !empty($start_time_f)){
...
header("Location: assign_new.php?error=2&s=$s&n=$n&t=$t&ta=$ta&start_d=$start_d&start_t=$start_t&deadline_d=$deadline_d&deadline_t=$deadline_t&start=$start_dtl&deadline=$deadline_dtl");
exit();}
if(empty($ta) && $ta != 0){
header("Location: assign_new.php?error=1&s=$s&n=$n&t=$t&ta=$ta&start_d=$start_d&start_t=$start_t&deadline_d=$deadline_d&deadline_t=$deadline_t&start=$start_dtl&deadline=$deadline_dtl");
exit();}
if(!empty($ta) && $ta <= 0){
header("Location: assign_new.php?error=3&s=$s&n=$n&t=$t&ta=$ta&start_d=$start_d&start_t=$start_t&deadline_d=$deadline_d&deadline_t=$deadline_t&start=$start_dtl&deadline=$deadline_dtl");
exit();}
if(!empty($ta) && !ctype_digit($ta)){
header("Location: assign_new.php?error=4&s=$s&n=$n&t=$t&ta=$ta&start_d=$start_d&start_t=$start_t&deadline_d=$deadline_d&deadline_t=$deadline_t&start=$start_dtl&deadline=$deadline_dtl");
exit();}
if(!empty($start) && $now > $start_tz){
header("Location: assign_new.php?error=5&s=$s&n=$n&t=$t&ta=$ta&start_d=$start_d&start_t=$start_t&deadline_d=$deadline_d&deadline_t=$deadline_t&start=$start_dtl&deadline=$deadline_dtl");
exit();}
if(!empty($ddl_tz) && $now > $ddl_tz){
header("Location: assign_new.php?error=6&s=$s&n=$n&t=$t&ta=$ta&start_d=$start_d&start_t=$start_t&deadline_d=$deadline_d&deadline_t=$deadline_t&start=$start_dtl&deadline=$deadline_dtl");
exit();}
$n_db = $mysqli -> real_escape_string($n);
