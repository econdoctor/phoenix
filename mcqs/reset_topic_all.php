...
exit();}
if(!array_filter($_POST['reset_topics'])){
echo 'Please select at least one topic.';
exit();}
if(!empty($_POST['reset_topics'])){
foreach($_POST['reset_topics'] as $check){
$sql_del = "DELETE FROM phoenix_answers WHERE user_id = '".$member_id."' AND answer_type = '2' AND topic_id =  '".$check."'";
$res_del = $mysqli -> query($sql_del);
$sql_up = "UPDATE phoenix_questions SET question_refresh = '1' WHERE question_topic_id = '".$check."'";
