...
$sql_name = "SELECT team_shield FROM phoenix_assign_teams WHERE assign_id = '".$assign_id."' AND team = '".$assign_student_team."'";
$res_name = $mysqli -> query($sql_name);
$data_name = mysqli_fetch_assoc($res_name);
$team_shield = $data_name['team_shield'];
?>
<!doctype html>
<html>
<head>
...
<?php
if($_GET['error'] == 1){
echo '<p><b style="color: #820000;">Please choose a team shield.</b></p>';}
if($_GET['error'] == 2){
echo '<p><b style="color: #820000;">This team shield is already used.<br>Please choose another one.</b></p>';}
?>
<p>&nbsp;</p>
...
if($i == $team_shield){
?>
<td width="16.67%"><img src="./shields/shield_<?php echo $i; ?>.png" width="150" style="-webkit-filter: drop-shadow(0px 0px 40px white); filter: drop-shadow(0px 0px 20px white);"></a></td>
<?php
}
if($i != $team_shield){
$sql_count = "SELECT COUNT(*) FROM phoenix_assign_teams WHERE assign_id = '".$assign_id."' AND team_shield = '".$i."'";
$res_count = $mysqli -> query($sql_count);
$data_count = mysqli_fetch_assoc($res_count);
$nr_count = $data_count['COUNT(*)'];
if($nr_count == 0){
?>
<td width="16.67%"><a href="team_shield_db.php?assign_id=<?php echo $assign_id; ?>&team_shield=<?php echo $i; ?>"><img src="./shields/shield_<?php echo $i; ?>.png" width="150" style="-webkit-filter: drop-shadow(0px 0px 40px white); filter: drop-shadow(0px 0px 20px black);"></a></td>
<?php
}
if($nr_count > 0){
?>
<td width="16.67%"><img src="./shields/shield_<?php echo $i; ?>.png" width="150" style="-webkit-filter: grayscale(100%); filter: grayscale(100%); "></a></td>
<?php
}}
if($i == 6){
