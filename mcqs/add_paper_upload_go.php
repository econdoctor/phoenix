...
header("Location: add_paper_ms.php?paper_reference=$paper_reference");
exit();}}
$dir = './q/' . $paper_syllabus . '/' . $paper_id;
$target_dir = 'q/' . $paper_syllabus . '/' . $paper_id . '/';
$target_del = 'q/' . $paper_syllabus . '/' . $paper_id . '/*';
if(!file_exists($dir)){
mkdir($dir, 0777, true);}
$get_files = glob($target_del);
...
if(strlen($filename_pre) > 2 || !is_numeric($filename_pre) || $filename_pre < 1 || $filename_pre > 20){
echo "There's an issue with some file names. Please check them and try again.";
$uploadOk = 0;}
...
header("Location: add_paper_upload2.php?paper_reference=$paper_reference");
?>